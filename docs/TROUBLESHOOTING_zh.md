...
h88-chrome-mcp-bridge report

# 写入到文件
h88-chrome-mcp-bridge report --output mcp-report.md

# 直接复制到剪贴板
h88-chrome-mcp-bridge report --copy
```
...
1.1 **检查h88-chrome-mcp-bridge是否安装成功**，确保是**全局安装**的
...
```json
{
  ...
  "path": "/Users/xxx/Library/pnpm/global/5/.pnpm/h88-chrome-mcp-bridge@1.0.23/node_modules/h88-chrome-mcp-bridge/dist/run_host.sh",
  "type": "stdio",
  "allowed_origins": ["chrome-extension://hbdgbgagpkpjffpklnamcljpakneikee/"]
}
```

> 如果发现没有此清单文件，可以尝试命令行执行：`h88-chrome-mcp-bridge register`
...
- **Windows**: `%LOCALAPPDATA%\h88-chrome-mcp-bridge\logs\`（例如 `C:\Users\xxx\AppData\Local\h88-chrome-mcp-bridge\logs\`）
- **Linux**: `~/.local/state/h88-chrome-mcp-bridge/logs/`
...
或者运行 `h88-chrome-mcp-bridge doctor --fix` 来写入当前 Node 路径。
